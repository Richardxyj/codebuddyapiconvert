...
SENSITIVE_TERMS: list[str] = [
    ...
    # 通道检测词（2026-09-19 P-201 复测实证，code 11128 "Illegal API invocation
    # from an unapproved channel"）：官方 base_instructions 头部的 Codex/OpenAI
    # 身份短语触发后端通道识别。零宽插入后模型照常阅读，检测器失配。
    # 实证：原文 400 / ZWSP 后 200（probe_11128.py P1 vs P1b）。
    "Codex CLI",
    "Codex Desktop",
    "Codex",
    "codex-cli",
...
# 路径 token 不做零宽脱敏：模型会按 cwd/文件路径构造命令，ZWSP 会让路径不存在。
# 只保护明确的 Windows/POSIX 路径片段；普通句子仍走 _PATTERN。
...
# 2026-09-19 P-201：environment_context / permissions 两个块改为保真透传
# （Codex 原生语义：模型需要 cwd/沙箱/审批语义），从替换表中移除；
# 其余运行时块（collaboration_mode / skills / plugins / system-reminder）仍摘要。
...
def desensitize_text(text: str) -> str:
    ...
    for match in _PATH_TOKEN.finditer(text):
        pieces.append(_PATTERN.sub(
            lambda m: _zero_width_split(m.group(0)), text[pos:match.start()]
        ))
        pieces.append(match.group(0))
        pos = match.end()
    pieces.append(_PATTERN.sub(
        lambda m: _zero_width_split(m.group(0)), text[pos:]
    ))
    return "".join(pieces)
...
def _prune_runtime_fragments(role: str, text: str) -> str:
    ...
    if role == "system" and any(marker in pruned for marker in _CODEX_SYSTEM_MARKERS):
        if "You are Claude Code" in pruned:
            # Claude Code system：维持原有段落裁剪（非本客户端）
        ...
        # else: Codex base_instructions 保真透传（P-201 补丁 #10）——不裁剪，
        # 敏感词由 desensitize_text 零宽脱敏兜底。
...
def _compact_harness_message(role: str, content) -> str | None:
    ...
    2026-09-19 P-201 修复（补丁 #10）：Codex 原生语义类内容改为**保真透传**
    （base_instructions / AGENTS.md instructions / environment_context /
    permissions instructions），仅靠 desensitize_text 零宽脱敏防审核误伤，
    不再整体替换为摘要——对齐原生 Codex Desktop 体验（模型能看到完整
    官方基础指令与项目 AGENTS.md 规则）。仍保留摘要的类别：
    - Claude Code system（非本客户端）
    - skills / plugins 清单（体积大且与请求 tools 数组重复，属有意偏差）
    - collaboration_mode / system-reminder / claudeMd（非 Codex 原生类）
    """
    text = _content_to_text(content)
    if not text:
        return None
    if role == "system" and any(marker in text for marker in _CODEX_SYSTEM_MARKERS):
        ...
        return None  # Codex base_instructions：保真透传（P-201）
    if any(marker in text for marker in _PERMISSIONS_MARKERS):
        return None  # permissions instructions：保真透传（P-201）
    ...
    if role == "user" and _looks_like_harness_user_message(content):
        if (
            ...
            return None  # Codex 项目指令 / 环境上下文：保真透传（P-201）
        return (
